...
class GroqLLMClient:
    ...
    def decompose(self, payload: dict) -> dict:
        ...
        prompt = (
            ...
            "Ràng buộc:\n"
            "1. Mỗi câu con phải TỰ ĐỦ NGHĨA — nêu lại tên shop/sản phẩm và thị "
            "trường. Không dùng 'shop đó', 'đối tượng kia'.\n"
            "1b. NGÀY chỉ nêu khi câu gốc tự nêu ngày, HOẶC khi chính NGÀY là "
            "trục đang so (ví dụ 'ngày nào cao nhất' — mỗi ngày một bước, xem "
            "ví dụ 1). Khi câu gốc so sánh/cộng giữa các ĐỐI TƯỢNG (shop, "
            "thương hiệu, sản phẩm) mà không nêu ngày, mỗi đối tượng là ĐÚNG "
            "MỘT bước và câu con đó KHÔNG được thêm ngày — thêm một ngày cụ "
            "thể, hay tệ hơn một bước cho từng ngày trong dữ liệu, là ràng "
            "buộc câu gốc không nêu (xem quy tắc 5), và với `sum` nó còn đếm "
            "trùng cùng một đối tượng qua nhiều đợt thu.\n"
            ...
            "Ví dụ 1 — trục so sánh là NGÀY, mỗi ngày một bước:\n"
            ...
            "nhiêu?, ...]\n"
            "  combine: argmax\n"
            "Ví dụ 2 — trục so sánh là ĐỐI TƯỢNG, câu gốc không nêu ngày, mỗi "
            "đối tượng đúng một bước và KHÔNG bước nào mang ngày:\n"
            "  hỏi: thương hiệu X có nhiều listing hơn thương hiệu Y tại VN không\n"
            "  steps: [Số listing của thương hiệu X tại VN là bao nhiêu?, Số "
            "listing của thương hiệu Y tại VN là bao nhiêu?]\n"
            "  combine: compare\n"
            "Payload: " + json.dumps(payload, ensure_ascii=False)
        )
